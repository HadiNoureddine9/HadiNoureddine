...
const About = () => {
  const highlights = [
    { icon: <FaAws className="text-3xl" />, title: "AWS Certified Professional", description: "Solutions Architect & Developer Associate certified, specializing in serverless architecture and cloud-native solutions.", color: "from-orange-500 to-yellow-500" },
    { icon: <FaCode className="text-3xl" />, title: "Full-Stack Expertise", description: "Proficient across the entire stack - from React frontends to Node.js/Java backends, with a focus on scalable architecture.", color: "from-blue-500 to-cyan-500" },
    { icon: <FaRocket className="text-3xl" />, title: "Performance Optimizer", description: "Reduced deployment times by 60% and infrastructure costs by 70% through CI/CD automation and serverless architecture.", color: "from-purple-500 to-pink-500" },
    { icon: <FaLightbulb className="text-3xl" />, title: "Problem Solver", description: "Passionate about turning complex challenges into elegant, efficient solutions that drive real business impact.", color: "from-green-500 to-emerald-500" },
  ];

  const journey = [
    { year: "2022-2023", title: "Full-Stack Foundation", description: "Built full-stack applications with React, Java Spring Boot, Python Django, and AI integrations.", icon: <FaCode /> },
    { year: "2023-Present", title: "Enterprise Development", description: "Leading backend development at Darbuka, architecting serverless solutions on AWS.", icon: <FaRocket /> },
    { year: "2024-2025", title: "AWS Mastery", description: "Earned AWS Solutions Architect and Developer Associate certifications, mastering cloud architecture.", icon: <FaTrophy /> },
    { year: "2025", title: "Machine Learning Journey", description: "Completed intensive 20-week ZAKA AI program, building LLM-powered applications and autonomous AI agents.", icon: <FaGraduationCap /> },
  ];
  ...
  >
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
      {values.map((item, index) => (
        <ValueCard key={item.label} item={item} index={index} />
      ))}
    </div>
  </motion.div>
));
